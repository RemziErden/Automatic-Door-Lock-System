...
LiquidCrystal_I2C screen(0x27,16,2); // Screen type and size are defined. 
...
void setup() 
{
  screen.init();                     // The screen has been included.
  screen.backlight();                // The screen backlight is turned on.
  screen.setCursor(2,0);             // Display row and column number has been defined. 
  screen.print("Please Scan");       // Text has been written in the defined row and column on the screen.
  screen.setCursor(3,1);             // Display row and column number has been defined.
  screen.print("Your Card");         // Text has been written in the defined row and column on the screen.
...
void loop() 
{
  if(rfid.PICC_IsNewCardPresent())              // Has the card been scanned?
  {
    ...
    if(rfid.PICC_ReadCardSerial())              // Scanning the card.   
    {
      ...
      screen.clear();                          // The writings on the screen have been deleted.
      screen.setCursor(4,0);                   // Display row and column number has been defined.
      screen.print("WELCOME");                 // Text was written on the screen.
      ...
      screen.clear();                          // The writings on the screen have been deleted. 
      screen.setCursor(2,0);                   // Display row and column number has been defined.
      screen.print("Invalid Card");            // It is written that the card is invalid on the lcd screen.
      delay(1000);                             // The time has been added for the next post.
      screen.setCursor(3,1);                   // Display row and column number has been defined.
      screen.print("Try Again");               // Text was written on the screen.
